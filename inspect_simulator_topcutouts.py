...
DEVICE_SWIFT_PATH = REPO_ROOT / "Sources/TopCutout/TopCutoutCatalog+Device.generated.swift"
DEVICE_PATH_SWIFT_PATH = REPO_ROOT / "Sources/TopCutout/TopCutoutCatalog+Device+Path.generated.swift"
DEVICE_SCREEN_SWIFT_PATH = REPO_ROOT / "Sources/TopCutout/TopCutoutCatalog+Device+Screen.generated.swift"
DEVICE_DISPLAY_NAME_SWIFT_PATH = REPO_ROOT / "Sources/TopCutout/TopCutoutCatalog+Device+DisplayName.generated.swift"
...
def swift_top_cutout_kind_literal(kind: str) -> str:
    if kind == "none":
        return ".none"
    if kind == "notch":
        return ".notch"
...
def write_swift_paths_file(
    ...
    lines = [
        ...
        "    /// A precomputed SwiftUI path describing the device's sensor housing outline, when available.",
        "    ///",
        "    /// This path is generated from catalog data and is not available for every supported device.",
        "    /// Callers are responsible for positioning or scaling it to match their own drawing context.",
        "    public var sensorHousingPath: Path? {",
        "        switch self {",
    ]
    ...
    lines.append("}")
    DEVICE_PATH_SWIFT_PATH.write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
...
def write_swift_screen_info_file(devices: dict[str, dict]) -> None:
    ...
    for model_identifier, device in sorted(devices.items(), key=lambda item: model_identifier_sort_key(item[0])):
        ...
        lines.extend(
                ...
                "                topCutout: TopCutoutCatalog.TopCutoutInfo(",
                    f"                    kind: {swift_top_cutout_kind_literal(top_feature['kind'])},",
    ...
    DEVICE_SCREEN_SWIFT_PATH.write_text(
        "\n".join(lines).rstrip() + "\n",
        encoding="utf-8",
    )
...
def write_swift_display_name_file(devices: dict[str, dict]) -> None:
    ...
    DEVICE_DISPLAY_NAME_SWIFT_PATH.write_text(
        "\n".join(lines).rstrip() + "\n",
        encoding="utf-8",
    )
